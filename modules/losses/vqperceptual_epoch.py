# Discard! remove to vqperceptual_multidisc
# since we start adversarial training just at the begining, no need to check epoch
...
# we assume the disc_start denotes the start epoch, i.e. [0,1,2,3,4,...]
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start_epoch, codebook_weight=1.0, pixelloss_weight=1.0,
        ...
        self.discriminator_iter_start_epoch = disc_start_epoch
        if disc_loss == "hinge":
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx, current_epoch, last_layer=None, cond=None, split="train"):
        ...
        if optimizer_idx == 0:
            ...
            disc_factor = adopt_weight(
                self.disc_factor, current_epoch, threshold=self.discriminator_iter_start_epoch
            )
