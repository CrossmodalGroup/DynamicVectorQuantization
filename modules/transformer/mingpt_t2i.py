...
class T2IGPT(nn.Module):
    """  the full GPT language model, with a context size of block_size  """
    """  add segmentation embedding to text and image respectively  """
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
        ...
        self.seg_emb = nn.Embedding(2, config.n_embd) # to identify text and image
    ...
    def forward(self, idx, embeddings=None, targets=None, segment=None):
        # forward the GPT model
        token_embeddings = self.tok_emb(idx) # each index maps to a (learnable) vector
        
        if segment is not None:
            token_embeddings = token_embeddings + self.seg_emb(segment)
