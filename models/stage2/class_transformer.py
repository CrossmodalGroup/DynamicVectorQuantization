...
from utils.utils import Labelator, instantiate_from_config
from models.stage2.utils import learning_rate_schedule, disabled_train

class ClassTransformer(pl.LightningModule):
    def __init__(self, 
                 transformer_config,
                 first_stage_config,
        ...
        print(f"Using class label as condition.")
        self.cond_stage_model = Labelator(n_classes)
    ...
    def avoid_class_sampling(self, logits):
        batch_size = logits.size(0)
        out = logits.clone()
        for i in range(batch_size):
            out[i, :self.n_classes] = -float('Inf')  # avoid sample class token
        return out
    ...
    def sample(self, x, c, steps, temperature=1.0, sample=False, top_k=None, top_p=None,
        ...
        for k in range(steps):
            ...
            logits = self.avoid_class_sampling(logits)
