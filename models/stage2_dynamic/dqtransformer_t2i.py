...
class Dualformer(pl.LightningModule):
    def __init__(self,
        ...
        super().__init__()
        self.first_stage_key, self.cond_stage_key, self.cond_stage_mask_key = "image", "caption", "mask"
        ...
        self.cond_stage_model = instantiate_from_config(config=uncond_stage_config)
        ...
        self.content_sos_code = uncond_stage_config["params"]["coarse_sos"]

        self.fine_position_sos_code = uncond_stage_config["params"]["coarse_pos_sos"]
        ...
        self.fine_position_sos_code = uncond_stage_config["params"]["fine_pos_sos"]
    ...
    def get_xc(self, batch, N=None):
        ...
        text_mask = self.get_input(batch, self.cond_stage_mask_key)
        if N is not None:
            x = x[:N]
            c = c[:N]
        return x, c, text_mask
    ...
    def forward(self, x, c, text_mask):
        ...
        output = self.transformer(
            ...
            cond = text_features, cross_mask = text_mask
        )

        return output
    
    def shared_step(self, batch, batch_idx):
        x, c, text_mask = self.get_xc(batch)
        output = self(x, c, text_mask)
        return output
    ...
    def log_images(self, batch, temperature=None, top_k=None, top_p=None, top_k_pos=None, top_p_pos=None, **kwargs):
        ...
        N = 4
        x, c, text_mask = self.get_xc(batch, N)
    ...
    def sample_from_scratch(
            ...
            text_features = None, text_mask = None,
        ):
        if self.activate_sos_for_fine_sequence:
        ...
        while not torch.all(sample_coarse_flag.bool()):  # sample coarse grain code, torch.all(sample_coarse_flag) is True only if all elements are True
            ...
            position_hidden, position_logits = self.transformer.sample_coarse_position(coarse_content=x_coarse, coarse_position=x_pos_coarse, coarse_seg=x_seg_coarse, cond=text_features, cross_mask=text_mask)
            ...
            # sample content next
            _, content_logits = self.transformer.sample_coarse_content(coarse_content=None, coarse_position=x_pos_coarse, coarse_seg=None, position_hidden=position_hidden, cond=text_features, cross_mask=text_mask)
        ...
        if not fix_fine_position: # sample with sampled fine position
            ...
            while not torch.all(sample_fine_flag.bool()):  # sample coarse grain code, torch.all(sample_coarse_flag) is True only if all elements are True
                position_hidden, position_logits = self.transformer.sample_fine_position(coarse_content=x_coarse, fine_content=x_fine, coarse_position=x_pos_coarse, fine_position=x_pos_fine, coarse_seg=x_seg_coarse, fine_seg=x_seg_fine, cond=text_features, cross_mask=text_mask)                
                position_logits = position_logits[:, -1, :] / temperature
                ...
                # sample content next
                _, content_logits = self.transformer.sample_fine_content(coarse_content=x_coarse, fine_content=x_fine, coarse_position=x_pos_coarse, fine_position=x_pos_fine, coarse_seg=x_seg_coarse, fine_seg=x_seg_fine, position_hidden=position_hidden, cond=text_features, cross_mask=text_mask)
                content_logits = content_logits[:, -1, :] / temperature
            ...
            for fine_index in trange(transfered_remain_fine_position.size(1)):
                ...
                # sample content next
                _, content_logits = self.transformer.sample_fine_content(coarse_content=x_coarse, fine_content=x_fine, coarse_position=x_pos_coarse, fine_position=x_pos_fine, coarse_seg=x_seg_coarse, fine_seg=x_seg_fine, position_hidden=None, cond=text_features, cross_mask=text_mask)
                content_logits = content_logits[:, -1, :] / temperature
                content_logits = self.avoid_special_or_enforce_pad_for_content(content_logits, sample_fine_flag)
                if top_k is not None:
                    content_logits = top_k_logits(content_logits, top_k)
    ...
    def avoid_special_or_enforce_pad_for_content(self, logits, flag):
        ...
        for i in range(batch_size):
            if flag[i] == 0:  # avoid special
                ...
                out[i, self.content_eos_code] = -float('Inf')
