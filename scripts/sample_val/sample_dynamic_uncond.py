...
if __name__ == "__main__":
    ...
    if opt.sample_with_fixed_pos:
        save_path_image_fixed = save_path + "fixed_TopK-{}-{}_TopP-{}-{}_Temp-{}_image".format(opt.top_k, opt.top_k_pos, opt.top_p, opt.top_p_pos, opt.temperature)
        save_path_pickle_fixed = save_path + "fixed_TopK-{}-{}_TopP-{}-{}_Temp-{}_pickle".format(opt.top_k, opt.top_k_pos, opt.top_p, opt.top_p_pos, opt.temperature)
        if opt.save_image:
            os.makedirs(save_path_image_fixed, exist_ok=True)
        os.makedirs(save_path_pickle_fixed, exist_ok=True)
    else:
    ...
    for i in trange(total_batch):
        ...
        if opt.sample_with_fixed_pos:
            ...
            samples_fixed_fine_position = model.decode_to_img(coarse_content, fine_content, coarse_position, fine_position)
            samples_fixed_fine_position = torch.clamp((samples_fixed_fine_position * 0.5 + 0.5), 0, 1)
            if opt.save_image:
                torchvision.utils.save_image(samples_fixed_fine_position, "{}/batch_{}.png".format(save_path_image_fixed, i))
            save_pickle(
                os.path.join(save_path_pickle_fixed, 'samples_({}_{}).pkl'.format(i, total_batch)),
                samples_fixed_fine_position.cpu().numpy(),
            )
        else:
            coarse_content, fine_content, coarse_position, fine_position = model.sample_from_scratch(
                    c_coarse, c_fine, c_pos_coarse, c_pos_fine, c_seg_coarse, c_seg_fine, 
